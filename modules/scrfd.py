"""this file implement the fd+5ptlms module, provide minimal gaze pre-request
cam-in->outer_preprocess->inner_preprocess->invoke->inner_post-process->outer_post-process->result
cam-in->resize_and_pad  ->normalize       ->invoke->decode_netout     ->scale the box     ->result
Note:
  - [ ] currently the tflite int file has large quantized error, but it can be solve by clip the conf output node, update soon.
  - [ ] it's seems cv2.dnn.NMS has some problem when all the input box has the same conf score
    ...
    - currently, just simple if nms output empty tuple, chose the first pred box auto.
"""
import os
import cv2
import time
...
class ResizeAndPad:
    def __init__(self, netin_shape, center=True, fillv=114, debug=False):
        """Resize/crop/pad cam-in_image to netin_image, outer pre-process for scrfd modules
...
class ScrfdFaceDetector(object):
    def __init__(self, int=False, conf_thr=0.5, iou_thr=0.9, debug=False):
        ...
        # load tflite model
        if int:
            #self.model_path = os.path.join("models", "fdlm", "scrfd500m_128_160_int8.tflite")  # 128 x 160
            self.model_path = os.path.join("models", "scrfd500m_128_160", "scrfd500m_128_160_int8.tflite")  # 256 x 320
            self.model_path = os.path.join("models", "quantize_fix", "scrfd500m_256_320_int8.tflite")  # 256 x 320
            #self.model_path = os.path.join("models", "quantize_fix", "scrfd500m_128_160_int8.tflite")  # 256 x 320
        else:
            #self.model_path = os.path.join("models", "fdlm", "scrfd500m_128_160_float32.tflite")  # 128 x 160
            self.model_path = os.path.join("models", "scrfd500m_128_160", "scrfd500m_256_320_float32.tflite")  # 256 x 320
        ...
        # outer (pre)processer
        self.outer_processor = ResizeAndPad(self.netin_shape, center=True, fillv=114, debug=debug)
    
    def outer_preprocess(self, cam_in):
        """Resize cam_in to netin_shape, a.k.a outer_preprocess
        ...
        """
        return self.outer_processor(cam_in)

    def inner_preprocess(self, image, auto_resize=False, BGR2RGB=True):
        """BGR2RGB, normalize, and expand_dim, a.k.a inner_preprocess

        Args:
            image (ndarray): usually cames from outer_preprocess
        ...
        if (self.source_height == self.netin_height) and (self.source_width == self.source_width):
            ...
            print("-----inner_preprocess: get the unmatching image shape, process resize in the inner process, aspect ratio may not keep")
    ...
    def inner_post_process(self, netout, top_k=1, debug=False):
        ...
        if debug:
            print("---score list from inner_post_process: ")
            pprint(scores_list)
        ...
        # nms
        if debug:
            print("[scrfd.inner_post_process]: before-nms, bboxes has shape: {}".format(bboxes.shape))
        ...
        if debug:
            print("[scrfd.inner_post_process]: keep: {}".format(keep))
...
#        kpss_keep = kpss[keep]
    ...
    def inner_inference(self, netin_image, debug=False):
        """Given image with net-in shape, inner_inference output the corresponding box directly.
        ...
        """
        start = time.time()
        netin = self.inner_preprocess(netin_image)
        invoke_start = time.time()
        netout = self._invoke(netin)
        invoke_end = time.time()
        boxes, lms = self.inner_post_process(netout, debug=debug)
        end = time.time()
        if debug:
    ...
    def outer_post_process(self, bboxes, lms, debug=False):
        ...
        """
        ratio = self.outer_processor.r
        (top, bottom, left, right) = self.outer_processor.padding_values
        if debug:
    ...
    def inference(self, image, debug=False):
        ...
        Returns:
            ...
            lms (ndarray): in [cam-in scale] with shape (num_pred, 5, 2), indicate 5 pts landmark in the [x, y] order.
        """
        netin_image = self.outer_preprocess(image)
        if debug:
            print("[scrfd.inference]: netin_image has shape: {}".format(netin_image.shape))
        boxes, lms = self.inner_inference(netin_image, debug=debug)
        if debug:
            print("[scrfd.inference]: after inner_inference: \n    boxes shape: {}, lms shape: {}".format(boxes.shape, lms.shape))
        boxes, lms = self.outer_post_process(boxes, lms, debug=debug)
        if debug:
            print("[scrfd.inference]: after outer_post_process: \n     boxes has shape: {}, lms has shape: {}".format(boxes.shape, lms.shape))
        return boxes, lms
    ...
    def plot(self,
        ...
        for box_idx, (box, kp) in enumerate(zip(boxes, kps)):
            ...
            cv2.putText(im2show, str(round(conf, 2)), (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, thickness=1)
            for lm_idx in range(5):
                cv2.circle(im2show, (int(kp[lm_idx, 0]), int(kp[lm_idx, 1])), 2, color, thickness=-1)
                if num:
                    cv2.putText(im2show, str(lm_idx), (int(kp[lm_idx, 0]), int(kp[lm_idx, 1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, thickness=1)
...
if __name__ == "__main__":
    ...
    # Note: assert you are in the main directory(not modules folder)
    # Except usage: python modules/scrfd.py
    ## model initial
    fd = ScrfdFaceDetector(int=True, conf_thr=0.5, iou_thr=0.1, debug=True)
    ## data initial
#    image_path = os.path.join("data", "image11718_0.jpg")
    image_path = os.path.join("data", "image63853_0.jpg")
    image = cv2.imread(image_path)
    im2show = image.copy()

    # infernece
    bboxes, lms = fd.inference(image, debug=True)
    fd.plot(im2show, bboxes, lms, num=True)
